...
class CreateProblemForm(FlaskForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.priority_id.choices = [
            (p.id, p.priority)
            for p in FlicketPriority.query.all()
        ]

        self.category_id.choices = [
            (c.id, "{} - {}".format(c.department.department, c.category))
            for c in FlicketCategory.query
        ...
        self.status_id.choices = [
            (s.id, s.status)
            for s in FlicketStatus.query.all()
        ]

        self.incidents.choices = [
            (t.id, "#{} - {}".format(t.id, t.title))
            for t in FlicketTicket.query.order_by(FlicketTicket.id.desc()).all()
        ]

    title = StringField(
        lazy_gettext('problem title'),
        validators=[
            DataRequired(),
            Length(
                min=field_size['title_min_length'],
    ...
    description = PageDownField(
        lazy_gettext('problem description'),
        validators=[
            DataRequired(),
            Length(
    ...
    root_cause = PageDownField(
        lazy_gettext('root cause')
    )

    resolution = PageDownField(
        lazy_gettext('resolution')
    )
    ...
    incidents = SelectMultipleField(
        lazy_gettext('related incidents'),
        coerce=int
    )
    ...
    submit = SubmitField(
        lazy_gettext('Create Problem'),
        render_kw=form_class_button
    )

class CreateKBArticleForm(FlaskForm):

    title = StringField(
        lazy_gettext('article title'),
        validators=[DataRequired()]
    )
    ...
    issue_type = StringField(
        lazy_gettext('issue type')
    )

    symptoms = PageDownField(
        lazy_gettext('symptoms')
    )
    ...
    keywords = StringField(
        lazy_gettext('keywords')
    )

    submit = SubmitField(
        lazy_gettext('Create Article'),
        render_kw=form_class_button
    )
