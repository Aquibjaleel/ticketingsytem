...
# view ticket details
@flicket_bp.route(app.config['FLICKET'] + 'ticket_view_detail/<ticket_id>/', methods=['GET', 'POST'])
@flicket_bp.route(app.config['FLICKET'] + 'ticket_view_detail/<ticket_id>/<int:page>/', methods=['GET', 'POST'])
@login_required
def ticket_view_detail_detail(ticket_id, page=1):
    ...
    if subscribers_form.sub_user.data and subscribers_form.validate_on_submit():
        ...
        return redirect(url_for('flicket_bp.ticket_view_detail', ticket_id=ticket_id))
    ...
    # Ai Logic in order to display ai summary
    ai_summary = None
    if current_app.config.get('AI_PROVIDER'):
        # Pass the ticket content to the sandbox
        ai_summary = AIService.get_summary(ticket.content)
