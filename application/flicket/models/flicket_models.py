...
class FlicketAction(PaginatedAPIMixin, Base):
    ...
    def output_action(self):
        ...
        user_name = escape(self.user.name) if self.recipient else None
        user_email = escape(self.user.email) if self.recipient else None
...
# Flicket KB Article table
class KBArticle(Base):
    """
    Knowledge Base Article model class
    """
    ...
    def __init__(self, title, resolution, category=None, issue_type=None, symptoms=None, keywords=None):
        self.title = title
        self.category = category
    ...
    def __repr__(self):
        return '<KBArticle: id={}, title={}>'.format(self.id, self.title)
    

# Flicker Problem ticket Table
class ProblemTicket(Base):
    __tablename__ = 'problem_ticket'

    id = db.Column(db.Integer, primary_key=True)

    problem_number = db.Column(db.String(100), unique=True, nullable=False, index=True)

    title = db.Column(db.String(field_size['title_max_length']), nullable=False, index=True)
    description = db.Column(db.Text, nullable=False)

    root_cause = db.Column(db.Text)
    resolution = db.Column(db.Text)

    priority_id = db.Column(db.Integer, db.ForeignKey(FlicketPriority.id), nullable=False)
    priority = db.relationship(FlicketPriority)

    category_id = db.Column(db.Integer, db.ForeignKey(FlicketCategory.id), nullable=False)
    category = db.relationship(FlicketCategory)

    status_id = db.Column(db.Integer, db.ForeignKey(FlicketStatus.id), nullable=False, default=1)
    status = db.relationship(FlicketStatus)

    created_by_id = db.Column(db.Integer, db.ForeignKey(FlicketUser.id), nullable=False)
    created_by = db.relationship(FlicketUser, foreign_keys=[created_by_id])

    modified_by_id = db.Column(db.Integer, db.ForeignKey(FlicketUser.id))
    modified_by = db.relationship(FlicketUser, foreign_keys=[modified_by_id])
    ...
    @property
    def department_category(self):
        if self.category and self.category.department:
            return f'{self.category.department.department} / {self.category.category}'
        return None

    def __repr__(self):
        return f'<ProblemTicket: id={self.id}, problem_number={self.problem_number}, title={self.title}>'
    
class ProblemIncident(Base):
    __tablename__ = 'problem_incident'

    id = db.Column(db.Integer, primary_key=True)

    problem_id = db.Column(db.Integer, db.ForeignKey(ProblemTicket.id), nullable=False)
    incident_id = db.Column(db.Integer, db.ForeignKey(FlicketTicket.id), nullable=False)

    problem = db.relationship('ProblemTicket', back_populates='incidents')
    incident = db.relationship(FlicketTicket)

    def __repr__(self):
        return f'<ProblemIncident: problem_id={self.problem_id}, incident_id={self.incident_id}>'